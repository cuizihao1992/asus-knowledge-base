...
范围：对 `E:\学习资料` 中的大文件、压缩包、安装包、下载残留和明显重复资料做浅层清单。本文只生成候选清单，不代表删除、移动、解压或上传。

## 总览

浅层扫描显示，`E:\学习资料` 的空间占用主要来自：

- 安装包：约 2.15GB，最大是 Android Studio 离线安装包。
- 压缩包：`.zip` 约 1.15GB，`.rar` 约 699MB，`.7z` 约 60MB。
- 视频：Cesium/CesiumLab/WebGL 演示视频和课程视频。
- 大 PDF/Word：书籍、论文、WebGL 资料和 Pushbroom 文档。
- 下载残留：`.qqdownload` 和 `.crdownload`。

当前建议是先保留资料资产，只把明显低价值或重复的内容列为候选。所有操作必须人工确认。

## 类型统计

| 类型 | 数量 | 浅层体量 | 默认处理 |
| --- | ---: | ---: | --- |
| `.exe` | 16 | 约 2.15GB | 工具索引/可删除候选，禁止运行 |
| `.zip` | 84 | 约 1.15GB | 压缩包清单，不自动解压 |
| `.rar` | 28 | 约 699MB | 压缩包清单，不自动解压 |
| `.7z` | 3 | 约 60MB | 压缩包清单，不自动解压 |
| `.qqdownload` | 10 | 约 50MB | 下载残留候选 |
| `.crdownload` | 2 | 约 3MB | 下载残留候选 |
...
| 文件 | 初步用途 | 建议 |
| --- | --- | --- |
| `android-studio-bundle-162.4069837-windows.exe` | Android Studio 旧安装包 | 可删除候选，需确认 |
| `opencv-3.3.0-vc14.exe` | OpenCV 旧安装包 | 可删除候选，需确认 |
| `3dtiles-gltf-explorer Setup 1.0.1.exe` | 3D Tiles/glTF 查看工具 | 工具索引，暂保留 |
| `COLLADA2GLTF-bin.exe` | COLLADA 转 glTF 工具 | 工具索引，暂保留 |
| `visualcppbuildtools_full.exe` | Visual C++ Build Tools | 可删除候选，需确认 |
| `osgb2tiles.exe` | OSGB 转瓦片工具 | 工具索引，暂保留 |
| `ImageStitching.exe` | 图像拼接工程编译产物 | 构建产物候选，需确认源码是否保留 |
| `vs_community__1685022915.1504234835.exe` | Visual Studio 安装器 | 可删除候选，需确认 |
| `UnityDownloadAssistant-2017.1.0f3.exe` | Unity 下载器 | 可删除候选，需确认 |
| `GIF录制录屏工具LICEcap.exe` | 录屏工具 | 工具索引，按需保留 |
...
重复候选只表示文件名和大小显示相同或高度相似，不代表可以直接删除。删除前需要哈希比对和人工确认。

| 资料 | 重复位置 | 建议 |
| --- | --- | --- |
| `WebGL 编程指南.pdf` / `WebGL编程指南.pdf` | `Cesium资料`、`Cesium资料\计算机_图形学`、`学习文档` | 保留一个主索引，删除需确认 |
| `WebGL.Insights.-.Patrick.Cozzi.pdf` | `webGL`、`Cesium资料\专栏_vtxf` | 高价值重复，保留索引 |
| `CESIUM基础培训代码(cesiumlab提供).zip` | `Cesium资料\计算机_图形学`、`学习文档` | 压缩包重复候选，不解压 |
| `CesiumlabHelp.pdf` | `Cesium资料\计算机_图形学`、`学习文档` | 文档重复候选 |
| `3DEngineDesignForVirtualGlobesSection121（level of details）.pdf` | `Cesium资料\计算机_图形学`、`学习文档` | 文档重复候选 |
| `卫星使用手册` 多个 PDF | `卫星使用手册`、`收集资料\卫星使用手册` | 目录重复候选 |
| `人类的性存在（第8版）.pdf` | `书籍`、`书籍\ipad` | 泛化书籍重复候选 |
| `CCD distortion calibration without accurate ground control data for pushbroom satellites.pdf` | `论文` 根目录和子目录 | 研究资料重复，谨慎处理 |
| `Linux命令大全` CHM/CHW | `linux命令`、`收集资料\linux命令大全` | 工具资料重复候选 |
| `GPU高性能运算之CUDA.pdf` | `CUDA\cuda编程学习`、`书籍` | 技术书重复候选 |

## 风险分级

| 风险级别 | 范围 | 处理建议 |
| --- | --- | --- |
| 高风险不可动 | 研究文档、论文、年终总结、Cesium/GIS 资料、卫星手册 | 只建索引和摘要 |
| 中风险待确认 | 压缩包、旧工具包、课程代码包、视频资料 | 只列清单，不解压 |
| 低风险候选 | 下载残留、临时文件、明显构建产物 | 生成候选清单后人工确认 |
| 可重新获取候选 | 旧安装包、通用工具安装器 | 确认后再删 |
...
当前不建议立即清理 `E:\学习资料`。更稳妥的顺序是：

1. 先保留高价值学习和研究资料。
2. 对重复 PDF 做哈希清单。
3. 对安装包、下载残留、构建产物生成待删除清单。
4. 人工确认后，再考虑实际删除或归档。

在确认前，不执行任何移动、删除、解压或上传。
